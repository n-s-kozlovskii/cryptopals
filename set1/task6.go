...
func EstimatedKeySizeAvg(a []byte, keySize int) float32 {
	var accDistance float32 = 0.0
	var iter float32 = 0.0
	for i := 0; i < len(a)/keySize; i = i + keySize {
		x := a[i:(i + keySize)]
		y := a[(i + keySize):(i + keySize*2)]
		accDistance += float32(HammingDistance(x, y)) / float32(keySize)
		iter++
	}
	return accDistance / iter
}
...
func FindSmallestKeySizes(src []byte) (int, int, int) {
	...
	for i := 2; i < 50; i++ {
		d := EstimatedKeySizeAvg(PrepareBytes(src, i, 50), i)
...
func SuperF(filename string) {
	...
	i1, i2, i3 := FindSmallestKeySizes(dat)
	bestScoredKeySize := 0.0
	// bestKey := []byte{}
	keysizes := []int{i1, i2, i3}
	for _, keysize := range keysizes {
		key := ConstrunctKey(dat, keysize)
		currentScore := float64(Score(string(key))) / float64(keysize)
		// fmt.Printf("key size: %d, score: %f\n", keysize, currentScore)
		if bestScoredKeySize < currentScore {
			bestScoredKeySize = currentScore
			// bestKey = key
		}
	}
}
